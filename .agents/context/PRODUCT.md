...
- Training settings include four form groups: Standard, Regional, Mega, and Gigantamax. All are enabled by default, including when loading older saves. Standard includes the retained alternate-form exceptions. A Pokémon must match both its form group and its introduction generation. Groups unavailable in the selected generations remain visible but disabled with an explanation, retaining their saved preference for when they become available again. Saving requires at least one available group selected. These filters apply to League Training and Custom Training targets, choices, and search. Daily and the Quizmon League challenge keep all form groups.
