...
Quizmon is a charming, complete browser game for testing broad Pokémon knowledge. Its signature mode is a new five-question Daily Challenge every local calendar day, with repeatable Training for the topics a player wants to practice and a demanding Quizmon League finale earned through long-term play.

## Positioning

Quizmon combines one shared Daily Challenge with a configurable training loop. Questions cover identification across current, historical, animated, and illustrated Pokémon imagery, Pokédex descriptions, types, matchups, abilities, moves, stats, and evolution changes, then finish with a search-first Champion question that offers optional clues.

## Operating Context

Daily is fixed at Level 3 across all generations, with one attempt per local calendar date. Training settings do not change Daily. Existing attempts and results retain their original rules, and retired challenge links cannot start a replacement lineup. Daily difficulty progression is deferred. Training has one repeatable ten-question flow with difficulty, independent generation/form controls, and optional customization. New players start at Level 1 with Gen I. Training shares one best across configurations for each scoring version. Results retain their original rules and saved multipliers. No account is required.

## Capabilities and Constraints

- Sprite match asks players to choose the unlabeled sprite of a named Pokémon. Who’s that Pokémon? asks players to name a silhouetted Pokémon using four text choices. Question families retain their identity across variants. Sprite option names and silhouette colors reveal after answering.
- Pixel Peek sizes its square crop using the Pokémon’s longest visible dimension, with gentler zoom for small sprites and tighter crops for large sprites. For a longest dimension L normalized to the sprite canvas, the crop width is L^0.8 / 3.5. Catalog generation checks sprite pixels and stores focus points across the silhouette, including extremities. Each focus point sits on a visible pixel and its crop contains at least 15% visible pixels. Seeded gameplay chooses from those points and reveals the full sprite after answering. Saved questions retain their crop and zoom.
...
- Pokémon sampling weights each eligible species by its highest-weight eligible category: ordinary 4, regional 2, or Mega/Gigantamax 1. Species-level repeat avoidance orders species within the same weight class, preserving those rarity probabilities. It then chooses an eligible form category with ordinary, regional, and Mega/Gigantamax weights of 4:2:1. Categories share their weight across their available forms, so additional variants do not increase a species or category’s chance. Targets and Pokémon answer choices use this order, and similarity shortlists retain the eligible forms of each selected species. Daily rotates weighted species slots deterministically and applies the same category weights afterward. Recency comparisons between assembled questions preserve the first valid draft’s category counts. Generation, description, matchup, and distinct-appearance constraints remain in effect. Existing form-keyed question history is interpreted at species level without rewriting saved lineups. Partner search and completion requirements continue to include every retained form.
- Training settings include four form groups: Standard, Regional, Mega, and Gigantamax. All are enabled by default. Standard includes the retained alternate-form exceptions. A Pokémon must match both its form group and its introduction generation. Groups unavailable in the selected generations remain visible but disabled with an explanation, retaining their saved preference for when they become available again. Saving requires at least one available group selected. These filters apply to Training targets, choices, and search. Daily and the Quizmon League challenge keep all form groups.
- Ability effect questions use imported short descriptions for all abilities with usable text. Choices use the same battle generation and exclude duplicate or closely overlapping descriptions. Authored advanced scenarios remain available, with imported questions as the fallback. Answer feedback highlights the correct choice without an additional explanation disclosure. Missing or placeholder descriptions are recorded as data gaps.
- Builds a versioned local knowledge catalog from PokéAPI through `pokenode-ts`; live rounds do not depend on API requests.
- Type twins always uses a dual-type target and asks for one Pokémon with exactly the same two types, regardless of their order. Every answer comes from a different evolution family than the target, including split branches. Targets without an unrelated type match are skipped. All option types and the target’s types reveal after answering. Legend hunt asks players to select every Legendary or Mythical Pokémon and reveals every option’s classification afterward. Training and Daily use the applicable variant rules.
- Evolution questions use explicit form links in the shipped catalog, with grouped endpoints resolved during catalog generation. Evolution Shift requires one destination that gains exactly one type, so branches with multiple destination forms are excluded. Type twins compares the shared species evolution family across all forms. Field notes prefers a form’s own English main-series description. Original forms may use species descriptions from before their first retained alternate form appeared. Missing form descriptions stay unavailable, including for Field notes distractors. Pokémon choices exclude shared sprite URLs and the identical Gigantamax Flapple/Appletun appearance. Picture and description choices exclude other forms of the target species, and Champion targets and search choices require a description that distinguishes the form. Champion search also excludes other forms of the target species, since even an explicitly assigned form description can describe its species generally.
- Generation roundup asks players to select every Pokémon introduced in a named generation, using at least two selected generations. It conceals Pokédex numbers and reveals every option’s generation after submission. Evolution link uses four name-only choices to complete an unambiguous three-stage chain. Its prompt, choices, and completed chain contain no photos or Pokédex numbers. Training and Daily use the applicable variant rules. Single-generation rounds skip Generation roundup.
- Upper levels rise sharply through closer alternatives, precise mechanics, complete answers, and less assistance. Every family remains eligible at Level 5 under its existing content filters, including inherited easier variants as occasional softballs. Height and weight thresholds remain provisional until player testing.
- Public difficulty UI uses numbered levels only. Never publish difficulty descriptions, family breakdowns, or level-to-family mappings. The executable internal matrix is centralized in `src/domain/quiz/question-variants.ts`; rule changes require a version bump. Saved lineups retain their own version and assistance state.
...
- Counts broad knowledge progress in both Daily and Training. Quick Attack and Perfect Form require ten questions with the same eligible-family configuration as automatic Training, including equivalent custom selections. There is no difficulty threshold. Champion’s Instinct requires description-only search without supplied or requested assistance.
...
- Gives every badge Bronze, Silver, and Gold tiers. Many Paths requires 1 correct answer in each of 10 formats, then 10 in each of 15, then 50 in each of 18 formats. Pokédex Trail requires 151 and 500 different correctly answered Pokémon, then completion of the entire Personal Pokédex against the shipped catalog. World Tour requires 1, 25, and 100 correct answers in each of all 9 generations. True Calling requires 50, 250, and 1,000 correct answers in one specialty. Quick Attack requires 1, 10, and 50 qualifying Training rounds. Perfect Form requires 3, 25, and 100 perfect qualifying Training rounds. Daily Resolve uses best Daily Combos of 3, 7, and 30 days. Champion's Instinct requires 1, 5, and 30 clue-free Champion answers.
- Gives all eight Trainer Titles tiers at 10, 100, and 1,000 correct answers in their specialty. Titles are equippable at tier 1 and automatically display their highest earned tier. Saved counters count toward higher tiers. Totals continue after Gold, and additional tiers do not change the League gate or Trainer rank.
...
- Training multiplies the knowledge, speed, and mastery total by difficulty (1–5), the number of unique selected generations, and the product of eligible selected question-type factors. A variant introduced at Level 1–2 contributes 0.75, Level 3 contributes 1, and Level 4–5 contributes 1.25. Identical inherited variants keep their original factor. Form selections have no separate factor. Round only the final score. Daily and League retain base scoring.
- Save format 6 is the prelaunch baseline. Versions 1–5 and retired standalone saves are unsupported. Never reset version counters. Future supported formats upgrade through validated, sequential load-time migrations. Malformed, retired, or newer saves block gameplay writes and open recovery with raw export, support contact, and current-backup restore. Malformed and retired saves also offer a confirmed reset. Preserve original bytes until an explicit restore or reset. Report unavailable storage separately.
