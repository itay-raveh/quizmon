...
- Evolution questions use only links valid for the default forms in the shipped catalog. Evolution Shift requires one evolution that gains exactly one type; branching species such as Eevee and Pokémon with alternate evolution forms are excluded. Field notes uses the newest available English description from an explicit main-series game list, including Let’s Go and Legends, with no spin-off fallback.
