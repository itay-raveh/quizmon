...
Play, Trainer, Rankings, and Account or Sign in form four equal main-navigation buttons, at the top on desktop and fixed to the bottom on phones. On desktop, the quiet Settings button sits directly to their left in one centered group; on phones it stays at the left of the header. Authentication has its own Account screen. Friends management belongs to the Friends view of Rankings. Trainer and result bodies do not repeat these navigation actions. From Daily results, the main Rankings action opens that challenge's exact date. Navigation and header utilities are hidden during live questions.
...
The signed-in Account screen leads with a visible Sign out action. Sync runs automatically and stays out of the way when healthy. A small red dot on Account navigation signals an error or unresolved change; the screen shows a compact recovery note only when needed. Do not display raw sync errors to players. Keep backup, restore, and the distinct non-restorable account archive together in Settings. Preserve the distinction between account progress and the separate guest save.
...
The shared footer stays in document flow on non-gameplay app screens and information pages. It has no background and uses compact spacing while keeping dark text legible over the landscape. Buy me a coffee is a yellow physical button, followed by the author and Contact line. Help, legal links, credits, and the brief rights notice begin below the initial viewport and are available by scrolling. Reserve the League action's landing-page space even before it unlocks, so unlocking never pushes the footer down. Keep each label and credit readable and intact when wrapping. Mobile spacing keeps the first footer line above the fixed navigation. Live questions omit the footer.
...
The footer is the same across the site: Buy me a coffee, the author credit, direct email and GitHub links, How to play, Privacy, and Terms, plus modest badge artwork and TextStudio wordmark credits. A short unofficial-project, non-affiliation, and rights line follows the credits. The existing `/about` page is titled How to play and explains the game. Full Nintendo and Pokémon ownership and affiliation notices live on the Terms page.
