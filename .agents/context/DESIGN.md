...
Play, Trainer, Rankings, and Account or Sign in form four equal main-navigation buttons, at the top on desktop and fixed to the bottom on phones. A compact Settings icon is a header utility, separate from the four destinations. Authentication has its own Account screen. Friends management belongs to the Friends view of Rankings. Trainer and result bodies do not repeat these navigation actions. From Daily results, the main Rankings action opens that challenge's exact date. Navigation and header utilities are hidden during live questions.
...
The Rankings surface presents Daily and Training as a primary tab-like switch. Global and Friends form a quieter second control row. Only Daily shows a UTC date field. Show one compact highlighted row for the player's rank when available, followed by a table with rank, Trainer, score, and time. Loading uses three restrained row placeholders; an empty board has one clear message and one next step. Standings refresh automatically while visible and on reconnection, with no manual refresh control. Manage friends remains a secondary action. Training uses each player's best eligible round from the current scoring version across settings. The existing navy outlines, cobalt selection, yellow accent, and short hard shadows carry the design without nested panels or extra copy.
