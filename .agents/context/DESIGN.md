...
Training exposes every format directly in Settings. Identity includes Pokédex scans across game sprites, animation, and illustration families, silhouette choices, cropped pixel peeks, and shiny spotting. Type play includes type checks, odd-one-out, select-all rounds, direct matchups, and Counter Pick. Field Notes, Evolution Shift, Ability Check, Move Check, and highest-or-lowest Stat Showdown add variety without turning the quiz into a deep data exercise. Multi-select choices turn yellow while composing an answer, keep checked markers visible, and use one explicit check action. Concealed or cropped artwork reveals its full sprite after submission. The Daily Challenge selects from the 15 core formats independently of Training settings, excluding abilities, moves, and stats.
