...
On desktop, the Settings and Customize training dialogs use `min(42rem, calc(100% - 1rem))`, cap their height to the safe viewport, and scroll only their bodies. On phones each becomes an edge-to-edge, full-height sheet so the title, content, and actions use the available viewport instead of nesting inside another frame. Each action footer remains visible. Every Training round contains ten questions. The generation picker leads the Training settings, followed by its concise mode description and the two-choice League or Custom control. League uses the 17 core formats without abilities, moves, or stats and permits its two performance badges. Custom offers all 20 formats and uses only those the player chooses. Only Custom reveals the question-type controls immediately after the mode choice. Identity, General knowledge, and Battle knowledge split those formats into collapsible semantic groups. Only one group opens at a time, and each heading reports its selected count so collapsed groups remain understandable. Question types use compact rows in one column on mobile and desktop. Each row aligns its selection mark, title, active difficulty factor, and separate help button. Selected rows show the same small yellow tick as Forms, without an outlined checkbox box. Unselected rows reserve the mark space without showing a tick. A type with no variant at the chosen difficulty shows Unavailable in place of its factor. Its checkbox is disabled and visually unchecked, using the same disabled surface and readable muted text as Forms. The saved selection preference is retained and restored when the type becomes available. Help remains available. A compact help button opens the selected format's explanation in a light-dismissable popover without hiding its selection. One select-all control serves the complete question-type set instead of repeating bulk actions inside every group. Validation appears beside the invalid section and moves focus there. Customize opens a separate Training dialog from the landing Training section. General Settings shows answer flow, timer, sound, and motion controls directly. Installation and reminders follow; Backup & restore opens a collapsed disclosure. There are no Settings tabs. Settings remains reachable through the shared header outside live questions. Active questions omit Settings and align the close control, question progress, and timer in one row. Multi-select options use full-label hit areas with native checkbox semantics, while help remains a separate action. On narrow screens, generations form a three-column grid and question types retain one column.
...
The footer is the same across the site: Buy me a coffee, the author credit, direct email and GitHub links, How to play, Privacy, and Terms, plus modest artwork, PokéAPI, and TextStudio wordmark credits. The existing `/about` page is titled How to play and explains the game. Full Nintendo and Pokémon ownership and affiliation notices live on the Terms page.
...
Customize training retains the generation/form grids, adds five numbered difficulty radios, and replaces the League/Custom switch with optional question customization. Difficulty labels never describe question types or rules. Results and shares use the saved configuration and multipliers. Training bests compare across settings within one scoring version. Current-format results retain their original scores. These requirements supersede the earlier Training-mode and Daily-layout descriptions above.
