...
Play, Trainer, Leaderboards, and Account or Sign in form four equal main-navigation buttons, at the top on desktop and fixed to the bottom on phones. On desktop, the quiet Settings button sits directly to their left in one centered group; on phones it stays at the left of the header. Authentication has its own Account screen. Friends management belongs to the Friends view of Leaderboards. Trainer and result bodies do not repeat these navigation actions. From Daily results, the main Leaderboards action opens that challenge's exact date. Navigation and header utilities are hidden during live questions.

The guest sign-in screen introduces account benefits as three compact bullet points. After a code is sent, show the destination email with a nearby change action, six visible code slots backed by one accessible input, an expiration hint beside the field, one full-width verification action, and a compact resend action with status nearby. Email and code fields show validation errors directly below the field, with an error border and an accessible description. Keep the sign-in card short and omit a closing privacy paragraph or a detached status line.

The signed-in Account screen leads with a compact sync state: one clear heading, a brief device-safety note, a visible indeterminate indicator only while connecting or syncing, and a count when changes are waiting. A failure replaces the loading state with Retry sync; expired authentication asks for sign-in instead. Keep raw error details and secondary export/sign-out actions behind disclosures. Preserve the distinction between account progress and the separate guest save.
...
The leaderboard surface presents Daily and Training as equal modes in one two-button switch. Global and Friends form a quieter second control row. Only Daily shows a UTC date field. Show one compact highlighted row for the player's rank when available, followed by a table with rank, Trainer, score, and time. Loading uses three restrained row placeholders; an empty board has one clear message and one next step. Refresh and Manage friends remain secondary actions. Training uses each player's best eligible round from the current scoring version across settings. The existing navy outlines, cobalt selection, yellow accent, and short hard shadows carry the design without nested panels or extra copy.
