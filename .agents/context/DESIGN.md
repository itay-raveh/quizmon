...
The guest sign-in screen introduces the account benefit in one brief sentence. After a code is sent, show the destination email with a nearby change action, six visible code slots backed by one accessible input, an expiration hint beside the field, one full-width verification action, and a compact resend action with status nearby. Email and code fields show validation errors directly below the field, with an error border and an accessible description. Keep the sign-in card short and omit a closing privacy paragraph or a detached status line.

The signed-in Account screen leads with a compact sync state: one clear heading, a short recovery note only when needed, a visible indeterminate indicator only while connecting or syncing, and a count when changes are waiting. A failure replaces the loading state with Retry sync; expired authentication asks for sign-in instead. Do not display raw sync errors to players. Keep secondary export/sign-out actions behind a disclosure. Preserve the distinction between account progress and the separate guest save.
...
The shared footer stays in document flow on non-gameplay app screens and information pages. It has no background and uses compact spacing while keeping dark text legible over the landscape. Links use weight to distinguish them and underline on hover or keyboard focus. Buy me a coffee is a quiet text link in its own row, followed by a compact wrapping line of author, Contact, help and legal links, and asset credits, then the brief rights notice. Keep each label and credit readable and intact when wrapping. Mobile spacing keeps it above the fixed navigation. Live questions omit the footer.
