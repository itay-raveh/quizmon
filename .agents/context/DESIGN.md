...
Every question uses the same instruction, stimulus, response, and action regions within the existing panel. The instruction stays directly below the heading. Minimum region sizes keep ordinary questions aligned while allowing long content and enlarged text to grow without clipping. Text choices use four rows on mobile and two columns on desktop. Pokémon choices share the same response height and reserve their full nameplates and revealed details, including concealed silhouettes. Short desktop screens retain the stacked structure rather than squeezing a second answer grid beside artwork.
