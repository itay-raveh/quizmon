...
| `src/domain/**/*.test.ts` | Pure game rule unit tests |
...
`mise run setup` also installs the Git hooks. Pre-commit checks formatting, lint, and secrets. Use conventional commit messages.

## Deployment

Pull requests and pushes to `main` run `.github/workflows/ci.yml`. After checks pass, a push to `main` calls `.github/workflows/release.yml` to build and publish the release image. GitHub does not deploy the Worker.
...
- `npm run dev` starts Vite, the account Worker, PostgreSQL, and PowerSync. Use `npm run dev:client` for the browser alone. Validate Worker bundles with `npm run deploy:dry-run`.
