...
Identity, evolution, and type knowledge can use playful interaction variants without adding settings categories. Identity includes reverse silhouettes, cropped pixel peeks, and shiny spotting. Evolution includes tap-in-order rounds. Type includes odd-one-out and select-all rounds. Multi-select and ordering choices turn yellow while composing an answer, keep numbered or checked markers visible, and use one explicit check action. Explanations appear only after submission, and concealed or cropped artwork reveals its full sprite at the same moment.
