...
The Settings dialog uses `min(42rem, calc(100% - 1rem))`, caps its height to the safe viewport, and scrolls only its body. Its action footer remains visible. Training contains generations, fifteen directly selectable question formats, and quiz length. Experience contains Quick transitions and sound effects. A two-control tab strip separates the sections without introducing another card layer. Settings remain reachable from the landing screen, every question, and results. Multi-select options use full-tile hit areas with native checkbox semantics and one Select all or Deselect all action per group. On narrow screens, generations form a three-column grid and question types form a wider two-column grid.
