...
The question surface is one warm panel anchored near the top of the available viewport over the unchanged landscape. Its top edge stays fixed while question lengths change, and the prompt reserves a modest responsive region so the answer field does not jump between short and longer copy. The progress row sits first, followed by a centered category title, mode, prompt, and answer grid. When a prompt names its subject, the Pokémon name uses bold Gabarito followed by a smaller, muted Martian Mono Pokédex number. The pair stays inline without a badge or added container. Sprite questions add unframed artwork with a modest drop shadow. The Champion question begins with an autocomplete search across the active Pokémon pool and no visible choices. Its first clue replaces search with the four-choice answer grid, while later clues use the ruled cream field.

An unfinished round resumes automatically after a reload in the same browser tab. A submitted answer is committed before its feedback delay, so reloading during feedback advances to the next unanswered question instead of granting another attempt. Leaving the game or completing it clears the resumable session.

Answer buttons use the primary control treatment and include small outlined number-key labels. Description, evolution, and stat questions present each Pokémon option as a cream field with a crisp front-facing PokéAPI pixel sprite and a cobalt nameplate containing its Pokédex number and readable Gabarito name. Type, ability, move, and matchup questions keep text answers and place one compact pixel sprite of the named Pokémon between the prompt and choices. Identification and Champion questions scale the same classic pixel sprites into larger subjects or silhouettes while preserving hard edges. Once selected, the correct answer becomes green with a visible check, the wrong selected answer becomes rose with a visible cross, unrelated answers fade to `0.5` opacity, and a silhouette immediately reveals its full-color sprite. When sound is enabled, controls use quiet tap and toggle cues, correct and incorrect answers use distinct short cues, completed or perfect rounds receive separate flourishes, and the animated score keeps its rolling count-up cue. Repeated interactions play one cue and interrupt their previous instance so sounds do not stack. Normal Training holds its answer state for `1.2s`, Daily Challenge for `0.85s`, and Quick transitions for `0.3s`. No visible post-answer text repeats the answer, score, or explanation. Assistive technology receives a concise hidden answer announcement.

Training exposes every format directly in Settings. Identity includes Pokédex scans, silhouette choices, cropped pixel peeks, shiny spotting, and back-sprite Battle View. Type play includes type checks, odd-one-out, select-all rounds, direct matchups, and Counter Pick. Field Notes, Evolution Shift, Ability Check, Move Check, and Stat Showdown add variety without turning the quiz into a deep data exercise. Multi-select choices turn yellow while composing an answer, keep checked markers visible, and use one explicit check action. Concealed or cropped artwork reveals its full sprite after submission. The strict Daily Challenge selects from every format independently of Training settings.
...
The Trainer Card is a first-class local profile reached from the landing screen and results. Its front makes the optional trainer name the title, presents the local six-digit number as a game-style `ID No.`, and places the partner Pokémon's name and Pokédex number directly beneath its square portrait. The join date, earned rank, chosen accent, and visible card finish complete the identity without explanatory finish copy. Its records face contains games completed, Daily clears, perfect rounds, the best Daily Combo, one qualified Specialty, and five evolving League stamps. Perfect Form rewards perfect Standard or Long Training rounds. Many Paths rewards correct answers across question formats. World Tour rewards correct answers across generations. Champion's Instinct rewards Champion answers without clues. Daily Resolve is the only stamp based on Daily Combo consistency. Each stamp has three tiers, and their combined tiers determine rank and card finish.

The compact records face shows five distinct pixel-cut motifs without labels or internal codes. Three small diamonds show each stamp's tier. The matching Stamp case beneath the records face names every stamp, states the next requirement, and shows progress. Earned marks and progress use the chosen card accent. Locked marks remain visible, but no stamp is awarded merely for completing a game. A meaningful new tier replaces the generic Trainer Card update on results with the stamp name, tier, and a route to its next challenge. Specialty requires at least ten answered questions in a category so a lucky first answer cannot claim expertise.
