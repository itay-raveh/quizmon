...
The Settings dialog uses `min(42rem, calc(100% - 1rem))`, caps its height to the safe viewport, and scrolls only its body. Its action footer remains visible. Training contains generations, fourteen directly selectable question formats, and quiz length. Experience contains Speedrun mode and sound effects. A two-control tab strip separates the sections without introducing another card layer. Settings remain reachable from the landing screen, every question, and results. Multi-select options use full-tile hit areas with native checkbox semantics and one Select all or Deselect all action per group. On narrow screens, generations form a three-column grid and question types form a wider two-column grid.
...
Training exposes every format directly in Settings. Identity includes Pokédex scans, reverse silhouettes, cropped pixel peeks, and shiny spotting. Evolution includes evolution trails and tap-in-order rounds. Type includes type checks, odd-one-out, and select-all rounds. Multi-select and ordering choices turn yellow while composing an answer, keep numbered or checked markers visible, and use one explicit check action. Concealed or cropped artwork reveals its full sprite after submission. The strict daily challenge selects from every format independently of Training settings.
